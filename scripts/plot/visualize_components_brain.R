...
for (j in 1:17) {
    components <- read.csv(paste0(indir, 'components/', netnames[j], '.csv'))
    ...
    print(plot(img$subjNet_mean, idx = 1:17, fname = paste0(outdir, netnames[j], '_components')))
}

for (j in 1:17) {
    for (i in 1:17) {
        weights <- readPNG(paste0(outdir, netnames[j], '_components_', img$subjNet_mean$meta$cifti$names[i], '.png'))
        assign(paste0('w', i), weights)
    }
    pdf(paste0(outdir, netnames[j], '_components.pdf'), width = 7, height = 12)
