...
# Columns
df2 <- df[complete.cases(df), ] #only 80 complete cases

na_counts <- colSums(is.na(df))
na_counts <- sort(na_counts, decreasing = TRUE)

na_counts #FC_int_somatomotora_salienceb_pos the highest with 232
...
#^ 50061 has the most missing neuroimaging data