...
network_membership <- engagements(networks_img, z = 2, verbose = TRUE, alpha = 0.01, method_p = 'fdr', type = '>')

saveRDS(network_membership, paste0(outdir, 'sub-', subid, '/ses-', sesid, '/network_membership_z2.rds'))
...
write.csv(df, paste0(outdir, 'sub-', subid, '/ses-', sesid, '/sub-', subid, '_ses-', 
                     sesid, '_surf_network_metrics_z2.csv'), row.names = FALSE)
