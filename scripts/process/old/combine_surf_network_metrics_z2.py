...
files = glob.glob(basedir+'/surfnet/sub*/ses*/*_surf_network_metrics_z2.csv')
df = pd.concat((pd.read_csv(f, header = 0) for f in files))

df.to_csv(basedir+'/tabulated/surf_network_metrics_z2_'+datetime.today().strftime('%Y-%m-%d')+'.csv', index=False)
