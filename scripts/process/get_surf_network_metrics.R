...
networks_img <- BrainMap(cii, prior, tvar_method = 'unbiased', hpf = 0,
                scale = 'local', TR = 2.05, scale_sm_FWHM = 2, GSR = FALSE) # July 16, 2025: Left off here
...
network_membership <- engagements(networks_img, verbose = TRUE, alpha = .01, method_p = 'bonferroni', type = 'abs >')
saveRDS(network_membership, paste0(outdir, 'sub-', subid, '/ses-', sesid, '/network_membership.rds'))
network_membership_pos <- engagements(networks_img, verbose = TRUE, alpha = .01, method_p = 'bonferroni', type = '>')
saveRDS(network_membership_pos, paste0(outdir, 'sub-', subid, '/ses-', sesid, '/network_membership_pos.rds'))
network_membership_neg <- engagements(networks_img, verbose = TRUE, alpha = .01, method_p = 'bonferroni', type = '<')
