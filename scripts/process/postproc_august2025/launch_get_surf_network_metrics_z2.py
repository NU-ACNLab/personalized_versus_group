...
for subdir in subdirs:
    ...
    if not os.path.exists(outdir + 'sub-' + subid + '/ses-1/sub-' +
                            subid + '_ses-1_surf_network_metrics_z2.csv'):
        cmd = ['Rscript /projects/b1108/projects/personalized_versus_group/scripts/process/get_surf_network_metrics_z2.R -s', subid, '-e 1']
        get_surf_network_metrics_script = launchdir+sub+'_'+ses+'_get_surf_network_metrics_z2_run.sh'
        ...
        os.system('sbatch -o '+launchdir+sub+'_'+ses+'_get_surf_network_metrics_z2.txt'+' '+get_surf_network_metrics_script)
